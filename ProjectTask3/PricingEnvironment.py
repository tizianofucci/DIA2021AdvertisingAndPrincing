...
class PricingEnvironment(Environment):
    ...
    def round(self,pulled_arm,bid):
        ...
        sales = np.sum(single_rewards)
        n_returns = np.zeros(int(sales))

        for i in range(0,int(sales)):
        ...
        production_costs = self.prod_cost*(total_returns + sales)
        self.total_returns_per_arm[pulled_arm] = np.append(self.total_returns_per_arm[pulled_arm],total_returns)
        money_reward = ((sales + total_returns) * self.prices[pulled_arm]) - costs - production_costs
        return money_reward
